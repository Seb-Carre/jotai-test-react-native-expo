...
export async function Sync(item: any | null = null) { // si on veut synchroniser une donnée ou juste faire une syncho générale
    const [data, setData] = useAtom(dataLevel)
    const checkConnection = await NetInfo.fetch() // récupération de l'état internet !
    if(checkConnection?.isInternetReachable) { // vérification de la connexion internet si la synchro est possible
        ...
        if(dataNotSync.length > 0) { // si il y a des éléments non synchro alors on envoie tout
            if(item) {
            ...
            if(request.status) {
                ...
                if(request.items){ // si l'api renvoie en retour un champ items alors il faut tout synchroniser sur l'appli
...
export function getModifiedAndNewItemsFromApi(dataFromApi : any[], data: any,  setData: any) {
